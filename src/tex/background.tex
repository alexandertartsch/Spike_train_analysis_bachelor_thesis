...
\begin{figure}
	...
	\caption{Typical mechanically and electrically stimulated recording in Spike2. There are multiple channels that display information. From bottom to top we can see the raw signal in $\mu V$, the Bath temperature in ºC, the mechanical force in N and a channel containing spikes belonging to one fiber. Another important channel is the Comments channel, which contains the experimental protocol from the experimenters. At the top the "DigMark" channel contains the timestamps for all of the electrical and mechanical events during the course of the recording.}
	\label{fig:spike2}
\end{figure}
...
\begin{figure}
	...
	\caption{A single spike train in the Spike2 software. At the top we can see the "DigMark" channel containing a mark for an electrical and mechanical event. The channel "nw-1" contains the spikes recorded during this period in $\mu V$. The "Force" channel displays the halfsine shape of the mechanical force applied in $mN$ and the "Signal" channel contains the recorded raw signal in $\mu V$.}
...
Looking at the literature, there is not one definite way to analyze a spike train, but different papers use different quantifications and statistics, depending on their set of experimental data.
